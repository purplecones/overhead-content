...
class LayoutTests(unittest.TestCase):
    ...
    def test_unknown_kind_key_is_reported(self):
        ...
        self.assertIn("content/index.json: unknown kind 'widgets'", messages(validate.validate(self.root)))
    ...
    def test_listed_entry_without_directory(self):
        ...
        self.assertIn("content/index.json: events/solar-eclipses lists '2024-04-08' but "
                      "content/events/solar-eclipses/2024-04-08/record.json is missing", found)
    ...
    def test_directory_not_listed(self):
        ...
        found = messages(validate.validate(self.root))
        self.assertIn("content/index.json: events/solar-eclipses/2024-04-08 exists but is not listed", found)

    def test_duplicate_listing(self):
        write_json(self.root / "index.json",
                   {"schemaVersion": 1, "kinds": {"events/solar-eclipses": ["2024-04-08", "2024-04-08"]}})
        found = messages(validate.validate(self.root))
        self.assertIn("content/index.json: events/solar-eclipses lists '2024-04-08' twice", found)
    ...
    def test_id_must_match_directory(self):
        ...
        found = messages(validate.validate(self.root))
        self.assertIn("content/events/solar-eclipses/2024-04-08/record.json: id is '2024-04-09' "
    ...
    def test_kind_must_match_key(self):
        ...
        found = messages(validate.validate(self.root))
        self.assertIn("content/events/solar-eclipses/2024-04-08/record.json: kind is 'body' "
    ...
    def test_stray_file_and_missing_readme(self):
        ...
        found = messages(validate.validate(self.root))
        self.assertIn("content/events/solar-eclipses/2024-04-08: README.md is missing", found)
        self.assertIn("content/events/solar-eclipses/2024-04-08/notes.txt: not declared by record.json", found)

    def test_unreadable_record(self):
        directory = write_entry(self.root, "events/solar-eclipses", "2024-04-08", {})
        (directory / "record.json").write_text("{not json")
        found = messages(validate.validate(self.root))
        self.assertTrue(any(m.startswith("content/events/solar-eclipses/2024-04-08/record.json: not valid JSON")
